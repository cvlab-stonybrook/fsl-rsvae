...
def main(config):
    ...
    utils.ensure_path(os.path.join(save_path, 'features'))
    ...
    for epoch in range(1, max_epoch + 1 + 1):
        ...
        if epoch == 1:
            out_file = os.path.join(save_path, 'features', 'train.hdf5')
            out_dir = os.path.join(save_path, 'features')
            print('Save training set features ...')
            save_features(model, train_loader, out_file)
            print('Trainig CVAE ...')
            save_vae_features(out_file, out_dir)
            vae_center = get_vae_center(out_dir, split='train')
        # train
        model.train()
        vae_center = torch.autograd.Variable(vae_center, requires_grad=True)
        center_optimizer = torch.optim.SGD([vae_center], lr=0.001, weight_decay=5.e-4)
        writer.add_scalar('lr', optimizer.param_groups[0]['lr'], epoch)
        
        for idx, (data, label) in enumerate(tqdm(train_loader, desc='train', leave=False)):
            data, label = data.cuda(), label.cuda()
            feats = model.encoder(data)
            feats_norm = F.normalize(feats, dim=-1)
            logits = model.classifier(feats)
            loss_entro = F.cross_entropy(logits, label)
            dists = -2 + torch.matmul(feats_norm, vae_center.T.cuda())
            log_exp_ndist  = torch.nn.functional.log_softmax(dists, dim=1)          
            yt = label.unsqueeze(1)
            gather_log_exp_dist = -log_exp_ndist.gather(1, yt)
            loss_attr = gather_log_exp_dist.squeeze().view(-1).mean()
            acc = utils.compute_acc(logits, label)
            loss = loss_entro + 0.5*loss_attr 
            optimizer.zero_grad()
            ...
            aves['tl'].add(loss_entro.item())
            aves['tl_attr'].add(loss_attr.item())
            aves['ta'].add(acc)
            if idx % 10 == 0:
                print('Step: %d: Entropy Loss: %.4f, Attr Loss: %.4f'% (idx, loss_entro.item(), loss_attr.item()))
            logits = None; loss = None; loss_attr = None
        ...
        if eval_fs and (epoch % ef_epoch == 0 or epoch == max_epoch + 1):
            ...
            save_vae_features(out_file, out_dir)
            vae_center = get_vae_center(out_dir, split='train')
            test_vae_center = get_vae_center(out_dir, split='test')
            fs_model.eval()
             
            for i, n_shot in enumerate(n_shots):
                np.random.seed(0)
                for data, label in tqdm(fs_loaders[i],
                                    desc='fs-' + str(n_shot), leave=False):
                    x_shot, x_query = fs.split_shot_query(
                            data.cuda(), n_way, n_shot, n_query, ep_per_batch=4)
                    ep_label = fs.make_nk_label(
                            n_way, n_query, ep_per_batch=4).cuda()
                    with torch.no_grad():
                        x_shot, x_query, metric = fs_model(x_shot, x_query)
                        label_real = label.view(4, n_way, -1)[:,:,0].view(-1)
                        vae_feats = test_vae_center[label_real].view(4, n_way, -1).cuda()
